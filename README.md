...
ImageButton.Image = "rbxassetid://16262294619"
...
spawn(function()
    ...
    if _G.CakePrince then
        if not string.find(QuestTitle, "Head Bakers") then
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
        end
        if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - FaiFaoQuestCakePrince.Position).Magnitude > 2500 then
            BTP(FaiFaoQuestCakePrince)
        elseif (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - FaiFaoQuestCakePrince.Position).Magnitude < 2500 then
               
            Tween(FaiFaoQuestCakePrince)
            end
      else
